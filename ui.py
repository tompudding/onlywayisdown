...
class FaderTextBox(TextBox):
    ...
    def Draw(self):
        ...
        glLoadIdentity()
        glTranslate(
            -globals.tiles.viewpos.Get().x - globals.tiles.width * globals.tile_dimensions.x,
            -globals.tiles.viewpos.Get().y,
            0,
        )
        glTranslate(self.absolute.bottom_left.x, self.absolute.bottom_left.y, 0)
        glScale(self.draw_scale, self.draw_scale, 1)
...
class TextBoxButton(TextBox):
    ...
    def SetVertices(self):
        ...
        # top bar
        self.hover_quads[0].SetVertices(
            Point(self.absolute.bottom_left.x, self.absolute.top_right.y - self.line_width),
            self.absolute.top_right,
            drawing.constants.DrawLevels.ui + 1,
        )
        # right bar
        self.hover_quads[1].SetVertices(
            Point(self.absolute.top_right.x - self.line_width, self.absolute.bottom_left.y),
            self.absolute.top_right,
            drawing.constants.DrawLevels.ui + 1,
        )

        # bottom bar
        self.hover_quads[2].SetVertices(
            self.absolute.bottom_left,
            Point(self.absolute.top_right.x, self.absolute.bottom_left.y + self.line_width),
            drawing.constants.DrawLevels.ui + 1,
        )

        # left bar
        self.hover_quads[3].SetVertices(
            self.absolute.bottom_left,
